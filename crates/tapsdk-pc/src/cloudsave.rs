...
impl CloudSave {
    ...
    pub fn list(&self, request_id: i64) -> Result<()> {
        let result = unsafe {
            tapsdk_pc_sys::TapCloudSave_AsyncList(self.handle, request_id)
        };
    ...
    pub fn create(&self, request_id: i64, request: &CreateSaveRequest) -> Result<()> {
        ...
        let extra_c = request.extra.as_ref()
            .map(|s| CString::new(s.as_str()))
            .transpose()?;
        let data_path_c = CString::new(request.data_file_path.to_string_lossy().as_ref())?;
        let cover_path_c = request.cover_file_path.as_ref()
        ...
        let raw_request = tapsdk_pc_sys::TapCloudSaveCreateRequest {
            ...
            extra: extra_c.as_ref().map(|s| s.as_ptr()).unwrap_or(std::ptr::null()),
            playtime: request.playtime,
            data_file_path: data_path_c.as_ptr(),
            cover_file_path: cover_path_c.as_ref().map(|s| s.as_ptr()).unwrap_or(std::ptr::null()),
